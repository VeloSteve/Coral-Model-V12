...
%option{5} = {'div', 5.33, 5.67};
option{5} = {'div', useLowerS, useUpperS};


options = [2, 3, 4, 5]; % Which variables may vary. Okay to include some with steps=1
steps = [1, 1, 1, maxSteps]; % Steps to actually use for each (1 to hold constant)
...
while runs < maxRuns && skips <= maxSkips && randomEnd < maxRandomEnd
    ...
    if ~good
        ...
        try
            ...
            thisPath = pwd;
            cd('..');
            [percentMortality, Bleaching_85_10_By_Event, C_seed, reefsThisRun, E] = A_Coral_Model(pd);
            cd(thisPath);
        catch ME
        ...
        [goodness, pg1950, bleach] = goodnessValueNew(bleachingTarget, ...
            percentMortality, Bleaching_85_10_By_Event);
...
[sBelow, sAbove] = showBests(cumResult, possible, steps);
optElapsed = toc(optTimerStart);
fprintf('Optimization finished in %7.1f seconds.\n', optElapsed);
fprintf('at %s\n', datestr(now));
% No longer possible unless psw2_new values are calculated just for this:
