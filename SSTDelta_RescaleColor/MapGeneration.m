...
function [] = MapGeneration(Reefs_latlon, values, figNum, tName, lowestScaleMax )
    ...
    cMap = flipud(customScale());
    oneMap(figNum, Reefs_latlon(:, 1), Reefs_latlon(:, 2), values, cMap, tName, lowestScaleMax);
...
function [] = oneMap(n, lons, lats, values, cMap, t, lowestScaleMax)
    ...
    for i = 1:length(LONG)
        ...
        reefColor = cMap(min(20, max(1, floor(cVals*(values(i)-minT)/tRange))), :);
