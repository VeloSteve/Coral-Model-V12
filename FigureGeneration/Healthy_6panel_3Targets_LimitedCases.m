...
path10 = '../FigureData/healthy_4panel_figure1/Mar2021_SimplerL2.6_0.5-1.5-0.32_Target10/';
...
for i = 1:panels
    ...
    for eee = 0:1
        for ooo = 0:1     
            % Skip E=0,OA=1, AND more if Target=10
            if ~(eee == 0 && ooo == 1) && ~(row==3 && ooo==1)  % Skip one curve  !!!!
    ...
    % each panel. SKIP FOR Target=10, which is row 3.
    eee = 0;
    if row < 3
        for ooo = 0:1           
