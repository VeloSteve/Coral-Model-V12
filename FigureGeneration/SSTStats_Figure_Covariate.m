...
for rrr = 1:length(rcpList)
    ...
    for E = [0 1]
        for adv = [0 1]
            adapt = E + 2*adv;
            % XXX bail for now:
            %if adapt == 3; fprintf("skip on 3, adapt = %d", adapt); continue; end
            %if adapt == 0; fprintf("skip on 0, adapt = %d", adapt); continue; end
            fprintf("\n\n === Case %s, E = %d, shuffling advantage %3.1f ===\n", ...
                rcp, E, adv);
            ...
            for endYear = endYearSet
                ...
                % In this version we retain low-variance reefs, so we don't
                % filter by idx2, but we do use it for grouping.
                idx = idx1;
                fprintf("For %s %d reefs meet area requirements and %d are high-variance (+%3.1f sd).\n", ...
                    rcp, nnz(idx1), nnz(idx2), stdOver);
                
                SST = SST_all(idx, :); 
                
                % We also need the variance flags, but just for the retained
                % reefs.
                idxVar = idx2(idx1);
                ...
                if nnz(idx) < 50
                    ...
                    covarPlots(sdSST_END, C_areaEND, idxVar, methodName{5}, caseID);

                    dt = sdSST_END-sdSST1900;
                    %% Row 4
                    rsqTable(rrr, 4, adapt+1, endYear) = ...
                        compareNoPlot(dt, C_areaEND, methodName{4}, caseID);
                    covarPlots(dt, C_areaEND, idxVar, methodName{4}, caseID);
                    ...
                    covarPlots(dt, C_areaEND, idxVar, methodName{1}, caseID);
                    ...
                    covarPlots(hotSdSSTEND, C_areaEND, idxVar, methodName{3}, caseID);
                    ...
                    covarPlots(dt, C_areaEND, idxVar, methodName{2}, caseID);
...
function rsq = compareNoPlot(stat, area, name, caseID)
    ...
    % for csv fprintf("%s | %s | %12.3f | %12.4f | %12.4f \n", caseID, name, rsq, b(2), b(1));
end
...
function covarPlots(measure, area, varGroup, methodName, caseID)
    fprintf("\n   == Stats for %s ==\n", methodName);
    %figure()
    %gscatter(measure, area, varGroup,'rbg','xo.', [], 'on', methodName, 'Coral Cover' )
    %title('Cover vs. ' + methodName + ', Grouped by Variance')
    %legend("Medium and low variance", "High variance");
    
    reefs = table(area, measure, varGroup);
    reefs.varGroup = categorical(reefs.varGroup);
    
    fit = fitlm(reefs, 'area~measure*varGroup')
    
    w = linspace(min(measure), max(measure));
    figure();
    gscatter(measure,area,varGroup,'rbg','xo.', [], 'on', methodName, 'Coral Cover' )
    line(w,feval(fit,w,'false'),'Color','r','LineWidth',2, 'DisplayName', 'Normal variance')
    line(w,feval(fit,w,'true'),'Color','b','LineWidth',2, 'DisplayName', 'High variance')
    title('Fitted Regression Lines by Variance Level ' + caseID)
    legend("Medium and low variance", "High variance");
