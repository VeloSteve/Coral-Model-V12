...
rcpList = {'control400', 'rcp45'};
rcpName = {'Control', 'RCP 4.5'};
...
for i = 1:columns
    ...
    sp = subplot(3, columns, i);
    plotSST(k, rcp, smooth, xRange, axisNums);
    addGenotype(k, giPath{i}, xRange, axisNums, legendColumn(1)==i);
    set(sp,'FontSize',14,'FontWeight','bold');

    sp = subplot(3, columns, i + 1*columns);
    plotSDensity(k, scPath{i}, xRange, axisNums, legendColumn(2)==i);
    set(sp,'FontSize',14,'FontWeight','bold');

    sp = subplot(3, columns, i + 2*columns);
    plotCPop(k, scPath{i}, xRange, axisNums, axisYears, legendColumn(3)==i);
    set(sp,'FontSize',14,'FontWeight','bold');
...
function plotSST(k, rcp, smooth, xRange, axisNums) 
    ...
    plot(time, T, 'Color', 'black', 'LineWidth', 2, 'DisplayName', 'SST');
...
function addGenotype(k, path, xRange, axisNums, showLegend) 
    ...
    plot(time, gi(:, 1), 'LineWidth', 2, 'DisplayName', 'Sensitive symbiont');
    plot(time, gi(:, 3), 'LineWidth', 2, 'DisplayName', 'Tolerant symbiont');
    xlim(firstDayNum(xRange));
    set(gca,'XTick', axisNums, 'XTickLabel', []);
...
function plotSDensity(k, path, xRange, axisNums, showLegend)
    % Load variables C, S, and time, all stored monthly.
    load(strcat(char(path), 'DetailedSC_Reef', num2str(k, '%d'), '.mat'), 'C', 'S', 'time', 'bleachEvent');
    inLegend(1) = plot(time, S(:,1)./C(:,1), '-r', 'LineWidth', 1, 'DisplayName', 'Mounding')
    hold on
    inLegend(2) = plot(time, S(:,2)./C(:,2), '-b', 'LineWidth', 1, 'DisplayName', 'Branching')
    inLegend(3) = plot(time, S(:,3)./C(:,1), '--r', 'LineWidth', 1, 'DisplayName', 'Mounding, 1C advantage')
    inLegend(4) = plot(time, S(:,4)./C(:,2), '--b', 'LineWidth', 1, 'DisplayName', 'Branching, 1C advantage')
    xlim(firstDayNum(xRange));
    set(gca,'XTick', axisNums, 'XTickLabel', [], 'XGrid', 'on');
    ...
    if showLegend
        ll = legend(inLegend, 'FontSize', 11, 'FontWeight', 'normal', 'Location', 'south')
        ll.Position = ll.Position + [-0.04 -0.015 0 0];
    end
end

function plotCPop(k, path, xRange, axisNums, axisYears, showLegend)
    ...
    if showLegend
        legend('FontSize', 11, 'FontWeight', 'normal', 'Location', 'south');
    end
