function  Healthy_4panel_2Targets()
...
path5 = '../FigureData/healthy_4panel_figure1/Mar2021_SimplerL2.6_0.5-1.5-0.32_Target5/';
path3 = '../FigureData/healthy_4panel_figure1/Mar2021_SimplerL2.6_0.5-1.5-0.32_Target3/';
...
figure('color', 'w');
set(gcf, 'Units', 'inches', 'Position', [2, 0, 10, 10]);
...
for i = 1:panels
    ...
    if i == 3
        % An extra lable to the left of the plot y labels
        text(1875, 45, 'Percent of healthy coral reefs globally', ...
...
posN = pos{rows*cols};
posNM = pos{rows*cols-cols};

% subplot positions are left, bottom, width, height
% Originally aligned the color bar to the top and bottom of the middle of 3
% rows.  Now shoot for halfway, vertically.
colorbar('Position', [posN(1)+posN(3)+0.025  posNM(2)-posN(4)/2  0.03  posN(4)], ...
     'Ticks', [0:1:3]*200/3, 'TickLabels', [0:1:3]); % Ticks for pcolor
% with contourf:          'Ticks', 0:1:3);

%colorbar('Position', [posN(1)+posN(3)+0.025  posN(2)  0.03  posNM(2)+posNM(4)-posN(2)], ...
%         'Ticks', 0:1:3);
% Add a label above the colorbar
annotation(gcf, 'textbox', ...
    [posN(1)+posN(3), posNM(2)+posNM(4)/2+0.02, 0.03, 0.03], ...
    'String', '\DeltaT(°C)', ...
...
function oneSubplot(X, Yset, T, legText, tText, useLegend, labelX, labelY) 
    ...
    if labelY
        ...
        set(gca, 'YTickLabel', [], 'YTick', [0 50 100]);
    end
