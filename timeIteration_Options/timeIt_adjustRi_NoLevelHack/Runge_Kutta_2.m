...
function [Snew, Cnew] = Runge_Kutta_2(Sold, Cold, i, dt, ri, ...
                            ...
                            alpha, KCx, Mu, um, KSx, G, expTune, adjustAllRi, superR)
    ...
    ctemp2 = ctemp / 2.0;
    rmk2  = a*exp(b*ctemp2.*superR); % maximum possible growth rate at optimal temp at t=i+0.5
    % XXX NOT compatible with superR != 1, set T 1 degree lower for super
    % symbionts. Note HARDWIRED value also not compatible with other advantage
    % levels.
    rmk2(3:4) = a*exp(b*(ctemp2-1));
    ...
    % Replace the passed min1 and min2 with Cheryl's new approach 18 Feb 2018
