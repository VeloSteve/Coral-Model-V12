...
function [S, C, gi, vgi, origEvolved, bleach] = timeIteration(timeSteps, S, C, dt, ...
        ...
        con, expTune, adjustRi, superRFraction)
    ...
    superR = repelem([1 superRFraction], SCratio);
    ...
    for i = 1:timeSteps
        rm  = con.a*exp(con.b*temp(i).*superR) ; % maximum possible growth rate at optimal temp
        % XXX:
        % Set super curve level with regular - NOT compatible with superR != 1
        rm(3:4) =  con.a*exp(con.b*(temp(i)-1));
        ...
        % Try Cheryl's M1/M2 definition which will keep ri > 0 and provide a
        % gradual change in the "kink" location.  SelV < 0.4 could still go
        % negative, but we can limit those cases.
        % Version 1 drops to 0 at SelVx == 1 and rises again.
        % min1 = sqrt(abs(1 - SelVx + vgi(i, :) + con.EnvVx));
        % Version 2 is better - drops to 0 at SelVx == 0
                    ...
                    for coral = 1:con.Cn
                        if bleach(simYear-1, coral)
                            ...
                            if sB || cB
                                ...
                                if superAdvantage > 0
                                    currentAdvantage = superAdvantage;
                                    shuffleGFactor(coral) = 1.0 - superGrowthPenalty;
        ...
        elseif (superMode == 9) && (con.Sn >= 2)
            % Apply a growth penalty proportional to the number of tolerant
            % symbionts.
            oms = 1.0 - superGrowthPenalty;
            shuffleGFactor(1) = (SiPlusOne(3) * oms + SiPlusOne(1)) / (SiPlusOne(3) + SiPlusOne(1));
            shuffleGFactor(2) = (SiPlusOne(4) * oms + SiPlusOne(2)) / (SiPlusOne(4) + SiPlusOne(2));
        end
