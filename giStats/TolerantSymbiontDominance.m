...
% E=0, A=1
figList = {'/ESM2M.rcp26.E1.OA0.sM9.sA1.0', ...
           '/ESM2M.rcp45.E1.OA0.sM9.sA1.0', ...
           '/ESM2M.rcp60.E1.OA0.sM9.sA1.0', ...
           '/ESM2M.rcp85.E1.OA0.sM9.sA1.0'};
runDate = '20210306';
baseDir = 'C:\CoralTest\Mar2021_SimplerL2.6_0.5-1.5-0.32_Target5\';
figList = strcat(baseDir, figList, '.', runDate, '_maps/SymbiontDominance.fig');
...
function  addOneCase(name, figFile, subplotB, subplotM, baseColor)
    ...
    oldLines = findobj(oldFig, 'Type', 'line');  % Massive was writen first

    % Lines do NOT consistently read back in the order they were created!
    if (contains(oldLines(1).DisplayName, 'Branching'))
        cB = copyobj(oldLines(1), subplotB);
        cM = copyobj(oldLines(2), subplotM);
    else
        cM = copyobj(oldLines(2), subplotM);
        cB = copyobj(oldLines(1), subplotB);
    end
